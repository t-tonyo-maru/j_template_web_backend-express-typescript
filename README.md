...
主に API サーバーを想定したテンプレートになっていますが、カスタマイズ次第で様々なパターンに対応できます。
...
**※package.json に`engines`を設定して、Node.js を v16 系に縛っています。**  
...
3. 必要に応じて環境設定ファイル（.env）を用意します
   - **※セキュリティ上の観点から、本番環境では.envファイルを読み込まないようにしています**
   - **※また、.env系のファイルはリポジトリに含めないようにしています。**
